...
def build_embedding(dictionary, embed_dim, is_encoder, path=None, padding_idx=-1):
    ...
    padding_idx = dictionary.pad() if padding_idx < 0 else padding_idx
...
class Transformer_nonautoregressive(FairseqModel):
    ...
    def build_model(cls, args, task):
        ...
        decoder = SelfTransformerDecoder(
            args, tgt_dict, decoder_embed_tokens, args.decoder_embed_scale
        )
...
class SelfTransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        self.project_in_dim = (
            nn.Linear(input_embed_dim, self.embed_dim, bias=False)
        ...
        self.embed_positions = (
            ...
            if not args.no_dec_token_positional_embeddings
            else None
        )
        ...
        # assert self.load_softmax is True

        # if self.load_softmax:
        if self.load_softmax and not args.no_softmax:
            assert args.adaptive_softmax_cutoff is None
            assert not self.share_input_output_embed
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, embed_scale=None, left_pad=False):
        ...
        self.embed_positions = (
            ...
            if not args.no_enc_token_positional_embeddings
            else None
        )
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        len_tokens = src_tokens.new(src_tokens.size(0), 1).fill_(0)
        print(len_tokens)
        ...
        # todo shape?
        print("emb len weights: %s" % str(self.embed_lengths.weight.shape))
        predicted_lengths_logits = torch.matmul(
            x[0, :, :], self.embed_lengths.weight.transpose(0, 1)
        ).float()
        print("pred len logits: %s" % str(predicted_lengths_logits.shape))
        print(predicted_lengths_logits)
    ...
    def reorder_encoder_out(self, encoder_out, new_order):
        ...
        if encoder_out["predicted_lengths"] is not None:
            encoder_out["predicted_lengths"] = encoder_out["predicted_lengths"].index_select(
                0, new_order
            )
        return encoder_out
...
def base_architecture(args):
    ...
    args.no_enc_token_positional_embeddings = getattr(
        args, "no_enc_token_positional_embeddings", False
    )
    args.no_dec_token_positional_embeddings = getattr(
        args, "no_dec_token_positional_embeddings", False
    )

...
class LabelSmoothedLengthCrossEntropyCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        loss, nll_loss, length_loss, ntokens = self.compute_loss(
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        length_target = (
            sample["net_input"]["prev_output_tokens"].ne(self.padding_idx).sum(-1).unsqueeze(-1)
        ...
        loss = (1.0 - self.eps) * nll_loss + eps_i * smooth_loss + length_loss
    ...
    def aggregate_logging_outputs(logging_outputs):
        ...
        return {
            ...
            "nll_loss": (
                sum(log.get("nll_loss", 0) for log in logging_outputs) / ntokens / math.log(2)
            ),
            "length_loss": (
                sum(log.get("length_loss", 0) for log in logging_outputs) / nsentences / math.log(2)
            ),
            "ntokens": ntokens,

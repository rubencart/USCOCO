...
class Dictionary(object):
    ...
    def index(self, sym):
        ...
        assert isinstance(sym, str) or isinstance(sym, int)
    ...
    def _add_file_to_dictionary_single_worker(
        ...
        assert worker_id == 0 and num_workers == 1
        counter = Counter()
    ...
    def add_file_to_dictionary(filename, dict, tokenize, num_workers):
        ...
        if num_workers > 1:
            ...
            merge_result(
                Dictionary._add_file_to_dictionary_single_worker(filename, tokenize, dict.eos_word)
            )

    @staticmethod
    def _add_sentences_to_dictionary(
        ...
        """
            # todo check if they add both training and val sentences to dict or only training?
        :param filename:
        :param dict:
        :param tokenize:
        :param caption_key:
        ...
        #     sentences: Iterable[str] = (d[caption_key] for d in data)
        for sentence in sentences:
            for word in tokenize(sentence.strip().rstrip(".").strip().lower()):
                counter.update([word])
            counter.update([dict.eos_word])

        merge_result(counter)


class TruncatedDictionary(object):
    def __init__(self, wrapped_dict, length):
        self.__class__ = type(
            wrapped_dict.__class__.__name__, (self.__class__, wrapped_dict.__class__), {}
        )

...
class ParallelEasyFirst(DecodingStrategy):
    ...
    def generate(self, model, encoder_out, tgt_tokens, tgt_dict):
        ...
        # todo first FW pass through DEC
        # tgt_tokens_2: (BS * beam) x T
        # token_probs: (BS * beam) x T
        tgt_tokens, token_probs, decoder_out = self.generate_non_autoregressive(
            model, encoder_out, tgt_tokens
        )
        ...
        for counter in range(1, iterations):
            if counter == 1:
                ...
                # sorted_probs, sorted_ordering: (BS * beam) x T
                # sorted_ordering[0][0] == 4 means token_probs[0][4] has greatest value of token_probs[0]
                sorted_probs, sorted_ordering = token_probs.sort(1, descending=True)
                # gen_order: (BS * beam) x T
                # gen_order[0][0] is the rank of the 1st element of token_probs[0] in sorted_probs[0]
                _, gen_order = sorted_ordering.sort(1)
            assign_single_value_byte(tgt_tokens, pad_mask, tgt_dict.pad())
            assign_single_value_byte(tgt_tokens, eos_mask, tgt_dict.eos())

            # todo next FW passes through DEC
            decoder_out = model.decoder(
                tgt_tokens, encoder_out, masking_type="easy_first_masking", gen_order=gen_order
            )
            ...
            non_eos_pad_mask = ~eos_mask.view(nb_sents, self.length_beam, seq_len) & ~pad_mask.view(
                nb_sents, self.length_beam, seq_len
            )
            ...
            converge_mask_sent = (
                converge_mask[torch.arange(nb_sents, device=converge_mask.device), max_idxes] > 0
            )
            ...
            if prev_converge_mask_sent is not None:
                ...
                tgt_tokens[~prev_converge_mask_sent] = new_tgt_tokens_reshaped[
                    ~prev_converge_mask_sent
                ]
